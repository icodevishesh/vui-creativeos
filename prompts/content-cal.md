...
- User will create calendar for a specific client
...
- Status-driven cell rendering — cells visually reflect member's name, task name, due date, task status (OPEN → OPENED → IN_PROGRESS → INTERNAL_REVIEW → CLIENT_REVIEW → APPROVED/REJECTED) so stakeholders see pipeline health at a glance.

- Bucket-level scheduling — date-picker to assign a bucket (and its tasks) to specific calendar dates, generating the visual content plan.
