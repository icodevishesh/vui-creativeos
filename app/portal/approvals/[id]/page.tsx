...
interface ReviewTask {
  ...
  subTasks: { id: string; title: string; description: string; status: string; createdAt: string; reviewerName?: string | null; reviewerType?: string | null }[];
...
function CopyRow({
  ...
  // Track status locally so buttons hide immediately after action
  const [localStatus, setLocalStatus] = useState(copy.status);
  const [localTaskStatusRow, setLocalTaskStatusRow] = useState(taskStatus);

  const activeTaskStatus = localTaskStatusRow || taskStatus;
  const activeCopyStatus = localStatus;

  const canAct = activeTaskStatus === "INTERNAL_REVIEW" || activeTaskStatus === "CLIENT_REVIEW";
  // A copy is already actioned for this stage if:
  //   - INTERNAL_REVIEW stage → copy moved to CLIENT_REVIEW (or beyond)
  //   - CLIENT_REVIEW stage   → copy moved to APPROVED/PUBLISHED
  const alreadyActioned =
    activeCopyStatus === "REJECTED" ||
    activeCopyStatus === "PUBLISHED" ||
    (activeTaskStatus === "INTERNAL_REVIEW" && (activeCopyStatus === "CLIENT_REVIEW" || activeCopyStatus === "APPROVED")) ||
    (activeTaskStatus === "CLIENT_REVIEW" && activeCopyStatus === "APPROVED");
  const isActionable = canAct && !alreadyActioned;

  const handleCopyAction = async (action: "approve" | "reject", feedback?: string) => {
    setIsActing(true);
    try {
      ...
      const res = await fetch("/api/approvals/copy", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          copyId: copy.id,
      ...
      setLocalStatus(data.copy.status);
      setLocalTaskStatusRow(data.task.status);
      onUpdated({ ...copy, status: data.copy.status });
      onTaskStatusChanged(data.task.status);
      toast.success(action === "approve" ? "Copy approved!" : "Copy rejected — sent back for revision");
      setRejectOpen(false);
      setRejectFeedback("");
    } catch (err: any) {
    ...
    <>
      <div className="bg-white border border-gray-100 rounded-lg overflow-hidden">
        ...
        >
          {/* Status dot */}
          <span
            className={`mt-1 w-2 h-2 rounded-full shrink-0 ${STATUS_DOT[activeCopyStatus] ?? STATUS_DOT.DRAFT}`}
          />
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                className={`ml-auto text-[10px] font-bold border px-2 py-0.5 rounded-full ${statusStyle(activeCopyStatus)}`}
              >
                {activeCopyStatus}
              </span>
            </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-2" onClick={(e) => e.stopPropagation()}>
            ...
            {isActionable && (
              <>
                <button
                  onClick={() => handleCopyAction("approve")}
                  disabled={isActing}
                  className="inline-flex items-center gap-1 px-2.5 py-1 text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 rounded-lg hover:bg-emerald-100 transition-colors disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={isActing}
                  className="inline-flex items-center gap-1 px-2.5 py-1 text-[10px] font-bold text-rose-500 bg-rose-50 border border-rose-100 rounded-lg hover:bg-rose-100 transition-colors disabled:opacity-50"
                >
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-400/30 focus:border-rose-300 resize-none"
            />
            <div className="flex items-center justify-end gap-3 pt-1">
              <button
                onClick={() => { setRejectOpen(false); setRejectFeedback(""); }}
              ...
              <button
                ...
                onClick={() => handleCopyAction("reject", rejectFeedback.trim())}
                className="px-5 py-2 text-sm font-bold text-white bg-rose-500 hover:bg-rose-600 rounded-xl transition-all disabled:opacity-50"
              >
...
export default function CalendarApprovalDetailPage({
  ...
  // Track task status locally so per-copy actions can update it
  const [localTaskStatus, setLocalTaskStatus] = useState<string | null>(null);
  const effectiveTaskStatus = localTaskStatus ?? reviewTask?.status ?? null;
  ...
  const actionMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Partial approval: some copies still pending — task stays in current stage
      if (data?.partial) {
        toast(data.message ?? "Some copies are still pending review.", {
          icon: "⏳",
        });
        return;
      }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2 border-t border-gray-50">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-semibold text-gray-800">
                {(reviewTask ?? calendar.tasks[0])?.project?.name ?? "—"}
              </p>
            </div>
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
            ...
            <div>
              ...
              <div className="flex items-start gap-2 flex-wrap">
                ...
                <button
                  ...
                  className={`flex flex-col items-start px-3 py-2 rounded-xl border transition-all min-w-[60px] ${
                    activeBucketId === null
                ...
                >
                  ...
                  <span className={`text-[10px] font-medium mt-0.5 ${activeBucketId === null ? "text-gray-300" : "text-gray-400"}`}>
                    {displayCopies.length} copies
                  </span>
                </button>

                {bucketsWithCopies?.map((bucket) => {
                  const count = displayCopies.filter((c) => c.bucketId === bucket.id).length;
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {filteredCopies.length > 0 && (() => {
              const approved = filteredCopies.filter(c => c.status === "APPROVED" || c.status === "PUBLISHED").length;
              const rejected = filteredCopies.filter(c => c.status === "REJECTED").length;
              const pending = filteredCopies.length - approved - rejected;
              return (
                <div className="flex items-center gap-2">
                  {approved > 0 && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-full">{approved} approved</span>}
                  {rejected > 0 && <span className="text-[10px] font-bold text-rose-500 bg-rose-50 border border-rose-100 px-2 py-0.5 rounded-full">{rejected} rejected</span>}
                  {pending > 0 && <span className="text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-full">{pending} pending</span>}
                </div>
              );
            })()}
          </div>
          {filteredCopies.length === 0 ? (
            ...
            filteredCopies.map((copy) => (
              <CopyRow
                ...
                onTaskStatusChanged={(s) => setLocalTaskStatus(s)}
              />
            ))
          )}
        </div>
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-6 space-y-4">
            ...
            <div className="space-y-3">
              {reviewTask.subTasks.map((sub, index) => {
                ...
                const clean = sub.description?.replace(/^(Approved — |Rejected — |Feedback — )/, "");
                      ...
                      <div className="min-w-0 space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={`shrink-0 px-3 py-1 text-[11px] font-bold uppercase tracking-wider rounded-full shadow-sm ${
                            ...
                            isRejected ? "bg-[#FFE4E6] text-[#E11D48]" :
                            "bg-[#FFF8E6] text-[#B45309]"
                          }`}>
                            {isApproved ? "Approved" : isRejected ? "Rejected" : "Feedback"}
                          </span>
                          {sub.reviewerName && (
                        ...
                        {!isApproved && <p className="text-xs font-medium text-gray-500 line-clamp-1">{clean}</p>}
                      </div>
                    </div>
